...
class BillingController extends BaseController
{
    public function index(): string
    {
        ...
        return $this->render('billing.index', ['invoices' => $invoices->fetchAll(\PDO::FETCH_ASSOC)]);
    }
    ...
    public function store(): void
    {
        Auth::requireAuth();
        $db = \App\Helpers\Database::connect();
        $db->prepare("INSERT INTO invoices (invoice_number, customer_id, invoice_date, due_date, po_number, subtotal, tax_rate, tax_amount, total_amount, notes, status, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")->execute([
    ...
    public function show(int $id): string
    {
        ...
        $items->execute([$id]);
        $payments = $db->prepare("SELECT * FROM invoice_payments WHERE invoice_id = ? ORDER BY payment_date");
        $payments->execute([$id]);
    ...
    public function update(int $id): void
    {
        Auth::requireAuth();
        $db = \App\Helpers\Database::connect();
        $db->prepare("UPDATE invoices SET invoice_number = ?, customer_id = ?, invoice_date = ?, due_date = ?, po_number = ?, subtotal = ?, tax_rate = ?, tax_amount = ?, total_amount = ?, notes = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?")->execute([
    ...
    public function recordPayment(int $id): void
    {
        Auth::requireAuth();
        $db = \App\Helpers\Database::connect();
        $db->prepare("INSERT INTO invoice_payments (invoice_id, payment_date, amount, payment_method, reference_number, notes, recorded_by) VALUES (?, ?, ?, ?, ?, ?, ?)")->execute([
            $id,
        ...
        Audit::log('Payment Recorded', 'invoice_payments', null, null, ['invoice_id' => $id, 'amount' => $_POST['amount']]);
